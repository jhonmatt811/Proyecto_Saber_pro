...
public interface TestIcfesResultRepository extends JpaRepository<RootEntity, Integer>{
        ...
        @Query(value = """
            SELECT 
                re.numero_registro AS "numeroRegistro",
                re.ciclo,
                re.year,
                e.nombre AS "nombre",
            ...
            WHERE 
                (:year IS NULL OR re.year = :year)
                AND (:ciclo IS NULL OR re.ciclo = :ciclo)
                AND (:programa IS NULL OR p.id = :programa)
